...
```{r male-female-histograms}
hist(mdims$hgt,main = "Male Height Chart",   xlab = "Height cm", ylab = "number of people", ylim=c(0,80))
hist(fdims$hgt,main = "Female Height Chart", xlab = "Height cm", ylab = "number of people")

summary(mdims$hgt)
summary(fdims$hgt)

#I would say the Male chart looks much closer to a bell curve as compared to the female chart which is heavier on the left tail.
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
It appears to be, but there's no way to tell without doing z tests to make sure 68, 95, and 97 for 1,2,3 sd's respctively.

```{r hist-height-ylim, eval=TRUE}
...
No, but it looks smoother than the Q-Q plot sans rnorm.  Stil waiting to see the stats!  The human eye
is prone to errors.
```{r sim_norm-prob-plot}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
Yes, not sure how many samples need to be removed to programatically prove normalness.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
Yes.  
```{r qqnormism-female-weight, eval=TRUE}
qqnormsim(fdims$wgt)
```
## Normal probabilities
```{r qqnorm-female-weight}
qqnorm(fdims$wgt)
qqline(fdims$wgt)
```
When looking closer, I see a tighter grouping between -1sd and 1sd, and a uniform flare-out.  I also don't see any values outside of 3sd's on either models.
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    *Question: So will the theoretical P be equivalent if the function is a normal?
    1. Do we uniformally remove values from either tail until we get the T(P) == P(norm)? 
    2. Should we select a value from each sd slice and then look at various T(P) and P(norms) for each quantile?
      i.e. q = mean, q = 182, q = 165, q = 140, q = 190
```{r pnorm-evals-height-vs-weight, eval=TRUE}
#female-height
heightEval <- 1 - pnorm(q = 182, mean = fhgtmean, sd = fhgtsd) - sum(fdims$hgt > 182) / length(fdims$hgt)

#before getting weight, I wanted to check the means
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)
#female-weight
weightEval <- 1 - pnorm(q = 65, mean = fwgtmean, sd = fwgtsd) - sum(fdims$wgt > 65) / length(fdims$wgt)

cat("height value: ", heightEval, "weight value: ", weightEval)
abs(heightEval - weightEval)
```

Height has a much closer approx value, to the third decimal.  And the Weight is only to the second.
* * *
...
```{r qqnorm-pelvic-elbow-age-chest}
bii.dimean <- mean(fdims$bii.di)
bii.disd   <- sd(fdims$bii.di)

hist(fdims$bii.di, main = "Female Height Chart", xlab = "Height cm", ylab = "number of people")


elb.dimean <- mean(fdims$elb.di)
elb.disd   <- sd(fdims$elb.di)

age <- mean(fdims$age)
age <- sd(fdims$age)

che.demean <- mean(fdims$che.de)
che.desd   <- sd(fdims$che.de)

```

```{r bii.di-histogram-qqnorm}
qqnorm(fdims$bii.di)
qqline(fdims$bii.di)
#B
