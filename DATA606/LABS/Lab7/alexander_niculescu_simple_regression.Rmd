...
The relationship looks linear, but only slightly positive.  Using a linear model might not deliver much insight.
...
scatter.smooth(mlb11$runs, mlb11$at_bats)
```
...
The relationship between the two variables is positive trending with a positive slope, with a not very strong relationship, and a few outliers.  The correlation is .6, a little better than neutral.
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    ...
    and its home runs?
Almost .8; this new correlation is much stronger.  Many runs are comprised of homeruns (would a union between homeruns and rbis return 1 for correlation?)
    
```{r summary-m2, eval=TRUE}
...
b_0         <- -2789.243
b_1         <-  .631
y_estimate  <- b_0 + b_1*x_mean 
m3 <- lm(runs ~ at_bats, data = mlb11)
plot(m3$residuals ~ mlb11$at_bats)
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?
Very poorly, I thought there would be a stronger correlation, but the R^2 value is smaller than .1
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    five).
bat_avg predicts best, as hits are a subset of runs anwway.
```{r all_summaries}
#plot_ss(x = mlb11$at_bats, y = mlb11$runs, showSquares = TRUE)
...
scatter.smooth(mlb11$runs, mlb11$bat_avg)
...
I think so.  new_obs seems to look like a percentage like a batting average, but maybe for rbis?
...
0.9349 is the R^2 for new_obs, so that is my best var.
