...
load("/Users/alejandro/Documents/R/DATA606/inst/labs/Lab2/more/kobe.RData")
head(kobe)
```
...
A streak of length 1 means a shot was hit then followed up by a miss.
A streak of length 0 is a miss. 
...
A typical streak length was 0, and the longest streak was 4.
...
```
A typical streak is again, 0.  And the longest streak is 6!  Most good basketball players shoot around 30 percent from the perimeter.  
...
I expect deviations to occur at the higher streaks, but still imagine 0 to be dominant due to house advantage for every throw.
For instance, I have no idea why I have a 6 streak, but no 4 or 5.  I just imagine tossing a weighted coin of sorts.
...
In both models the 1 streak is the strongest, next to the 0.  Which is not a hot hand model.  I don't think the hot hand model fits either shooting patterns.  Since 0 and 1 are both anti-hot hand model, it's easy to infer the hot hand model isn't true.
In order for the hot-hand model to be true, the transition from 1->2 and 2->3 should have a probability of over 45%, which they do
