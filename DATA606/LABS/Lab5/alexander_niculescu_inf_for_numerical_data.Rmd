...
Categorical : mature, weeks, premie, visits, marital, habit, whitemom
Numerical   : fage, mage, gender, weight, gained, lowbirthweight
...
boxplot(weight~habit,data=nc, main="Smoker vs Non Smoker", 
...
non-smoking mothers.
```{r weight-habit-ci, eval=TRUE, tidy=FALSE}
...
```{r weeks-ci-95, eval=TRUE, tidy=FALSE}
...
```{r weeks-ci-90, eval=TRUE, tidy=FALSE}
inference(y = nc$weeks, est = "mean", type = "ci", null = 0, 
...
```{r mean-weight-gained-young-mature-ci, eval=TRUE, tidy=FALSE}
...
```{r young-mature-agecutoff-ci, eval=TRUE, tidy=FALSE}
by(nc$mage, nc$mature, summary)

cat("For the lower bounds; I can use the min and max values(age 13-34 for young women) and (age 35 - 50 for older women), since there is no overlap")
```
...
Another interesting combination of a numerical and categorical variables could be the relationship between weeks of pregnancy and gender.  Is there any difference between boys and girls?

H(0)  : Mean(weeks | gender male) == Mean(weeks | gender female)
H(A)  : Mean(weeks | gender male) < Mean(weeks | gender female)
H(A)' : Mean(weeks | gender male) > Mean(weeks | gender female)


```{r mean-weightgained-young-mature-ci, eval=TRUE, tidy=FALSE}
inference(y = nc$weeks, x = nc$gender, est = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
by(nc$weeks, nc$gender, summary)
```
The means are off by a tenth, therefore we accept the null hypothesis that there is no difference in the weeks of pregnancy for carrying a boy or a girl.
