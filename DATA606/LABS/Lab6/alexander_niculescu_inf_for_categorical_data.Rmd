...
Population paramters; because the polls are trying to summarize population sentiment, not representing a sample.
...
We can assume stratified sampling or cluster sampling was used to use characteristics and features from which to pick samples.
...
Each row shows nationality, response, and year of degree of being religious.
...
The observations must be independant and randomly selected.  
...
```{r inference-Uzbekistan-Vietnam}

uz12 <- subset(atheism, nationality == "Uzbekistan" & year == "2012")
ca12 <- subset(atheism, nationality == "Canada" & year == "2012")


inference(uz12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

inference(ca12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

cat("Standard Error for Canada: ", 0.009*1.96)
cat("Standard Error for Uzbekistan: ", 0.0063*1.96 )
```

## How does the proportion affect the margin of error?

The relationship is hyperbolic in the x,y coordinate plane as an x^2 function with a normal distribution.
...
The relationship is hyperbolic in the x,y coordinate plane as an x^2 function with a normal distribution.
When ME is as its largest, it only peaks Proportion at the middle value.
...
```{r spain-athiesm2005-2012}
sp12 <- subset(atheism, nationality == "Spain" & year == "2012")
sp05 <- subset(atheism, nationality == "Canada" & year == "2005")
...
```{r us-atheism-change-2012-2005}
us05_12 <- subset(atheism, nationality == "United States" & year == "2005"  | nationality == "United States" & year == "2012")

inference(y = us05_12$response, x = us05_12$year, est = "proportion",type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
```
Yes, there has been a significant change.
...
```{r acceptance}
cat("expected change is number of countres * summary =", (.05 * 39))
```
...
```{r p_n_relationship}
relationsip_p_n<- 1.96^2*.5*(1-.5)/(.01^2)
cat("The relationship between p and n is: ", relationsip_p_n)
```
