...
The distribution is multimodal.  But perhaps we can split it into many normal distributions?
```{r describe-sample}
summary(samp)
hist(samp)
```
...
I expect it to be off, the sample size is too small.  It would deviate too much.  So, similar levels of variance.  So the error bounds
would be grouped precisely, but not accurately.
...
A 95 CI means point estimate +/- 1.96 X SE.  Which refers to the porportion to which the values exist inside the expected bounds and 
map to the mean.
...
Yes, mine does.  But the neighbor's interval is slightly off, if he were to take a sample.  samp is somewhere between 95-98 percent.
...
samp is somewhere between 95-98 percent.  There would be no way to check without doing a monte carlo simulation or to take many samples.
...
```{r ci50_80percent, eval=TRUE}
lower_vector80 <- samp_mean - 1.28 * samp_sd / sqrt(n) 
upper_vector80 <- samp_mean + 1.28 * samp_sd / sqrt(n)

c(lower_vector80[1], upper_vector80[1])
plot_ci(lower_vector80, upper_vector80, mean(population))
...
It is way off, but the average is still within the bounds selected at 95 percent.  Much closer to the upper bounds, but still close.
15 pounts have deviated, which for 80 percent makes sense.
