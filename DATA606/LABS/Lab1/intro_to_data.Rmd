...
20000 Cases, 9 Variables
Categorical {distinct groups}: `genhlth`, `gender`
Discrete {numerical}:`exerany`, `hlthplan`, `smoke100`
Continous {can be infinite}: `height`, `weight`, `wtdesire`, `age`
...
2. cont'd Compute the relative frequency distribution for
    `gender` and `exerany`. 
```{r frequency-gender-exerany}
rows <- 20000
summary(cdc$gender)
summary(cdc$exerany)
...
genderTable/rows
exeranyTable <- table(cdc$exerany/rows)
exeranyTable/rows
```
...
```{r males-excellenthealth}
table(cdc$genhlth)[1]/rows
```
...
There is a higher chance that Men smoke more than 100 ciggarettes in their lifetime, and less than 50 percent for women.
P(Man smokes more than 100 in a lifetime) = 5022 / (4547 + 5022)
P(Woman smokes more than 100 in a lifetime) = 4419 / (6012 + 4419)
...
boxplot(bmi ~ cdc$smoke100,main="BMI and Smoking More than a hundred ciggarettes",xlab="Smoked more than 100 cigarettes",ylab="BMI")

boxplot(bmi ~ cdc$gender,main="BMI and Gender",xlab="gender",ylab="BMI")

```

Neither ciggarette smoking nor gender seem to reveal much about BMI's.  In school we were taught that normal is ~17 for boys and ~22 for women.  So I thought there was a clear difference which would show on the box plot.  However, it turns out women's BMI's are lower across all ages (maybe the difference only exists during adolescence then normalizes over time?)  Also, the ciggs were a stretch - but I had the idea that any indication of smoking might show skewed results.  
...
```{r weight-desiredWeight, eval=TRUE}
plot(x=cdc$weight, xlab="weight",y=cdc$wtdesire,  ylab="desired weight")
```
...
If they are okay with their weight = 0, want to lose (negative), want to gain(positive)
generic hypothesis: women want to lose, men want to gain.
...
```{r boxplot weight-gender}
boxplot(wdiff ~ cdc$gender)

```
It seems as if men, in this sample, are much closer to their desired weight. Men also have tight groupings both under and over the zero value, but women only have tight groupings over.
