...
The 1st, 3rd Q, Median, and Mean are all very close.  The full set just has smaller/bigger min/maxes.
```{r distribution-samp1}
summary(samp1)
hist(samp1)
```
...
Our sample will show us a good range and we have a mean to compare and contrast against many subsets as well as the adctual mean (which is 1500.)
...
The 1000 size sample is most accurate, and samp2 is also more accurate than samp1, for no apparent reason.
```{r samp2-and-more-samples}
samp2 <- sample(area, 50)
samp3 <- sample(area, 100)
samp4 <- sample(area, 1000)
mean(samp1);mean(samp2); mean(samp3); mean(samp4)
```
...
The center is 1500
I expect the distribution to be off with the more samples added.  5000 is already 2n, where n is the original sample size.  The error bounds is 
heightened by the extended sampling.
```{r describe-sample_means50}
length(sample_means50)

summary(sample_means50)

sample_means50big <- rep(NA, 50000)

for(i in 1:50000){
   samp <- sample(area, 50)
   sample_means50big[i] <- mean(samp)
   }

summary(sample_means50big)
```
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    element represent?
Each element represents an index in the `sample_means_small` vector.  Each index is a mean of a sample from the `area` dataframe.  
There are 100 elements from 1 to 100.  
```{r sample_means_small}
...
cat("some sample means: ",  head(sample_means_small))
cat("size of the vector: ", length(sample_means_small))
```
...
summary(sample_means10); summary(sample_means50); summary(sample_means100)
...
The center moves towards a normal distribution / gaussian curve.  The spread gets tighter, the maxes and mins move closer to the mean.
* * *
## On your own
...
The point estimate is mean : 184982 
```{r price-sample}
price_samp <- sample(price, 50) 
  
summary(price); summary(price_samp)
```
...
```{r price_sample_means50, eval=TRUE}
sample_means50 <- rep(NA, 5000)
...
I would guesss 180833 as per the summary output below for `sample_means150`
```{r price_sample_means150, eval=TRUE}
...
summary(price);summary(sample_means50); summary(sample_means150)
```
...
2 has a smaller sprea, a smaller spread is always better for estimating true means and values.
<div id="license">
